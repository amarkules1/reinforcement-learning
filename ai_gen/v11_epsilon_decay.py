...
obstacle_interval = 3
game_started = False
epsilon = 1.0 if training_mode else 0 # Exploration rate for training (start high)
...
def choose_action_dqn(state):
    if training_mode and random.uniform(0, 1) < epsilon:
        ...
        action_index = np.argmax(q_values[0].numpy())
        return ["up", "down", "left", "right"][action_index] # Choose action with highest Q-value
...
def reset_game():
    ...
    if episode_reward > best_reward: # Check for new best reward based on episode reward
        ...
        if training_mode:
            print(f"Iteration: {iteration_number}, New best reward: {best_reward}, epsilon: {epsilon}")
...
while True:
    ...
    if render_ui: # Control game speed based on mode
        time.sleep(0.1 if not training_mode else 0.000001) # Original speed in eval, faster in training
    else:
