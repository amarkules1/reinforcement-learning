...
def go_up():
    if head.direction != "down":
        head.direction = "up"
        global game_started # Indicate game has started
        game_started = True
...
game_started = False # Game starts paused, obstacles off

# Main game loop
while True:
    screen.update()

    # Check for obstacle creation time, only if game started
    if game_started:
    ...
    if head.xcor()>290 or head.xcor()<-290 or head.ycor()>290 or head.ycor()<-290:
        ...
        head.goto(0,0)
        head.direction = "stop"
        game_started = False # Pause obstacle creation again
