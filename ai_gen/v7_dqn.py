...
discount_factor = 0.9
q_table_file = "q_table.h5" # Changed to .h5 extension for Keras weights file
...
def build_model():
    ...
    model.compile(loss='mse', optimizer=keras.optimizers.Adam(learning_rate=learning_rate)) # Adam optimizer, MSE loss
    return model
...
def update_dqn(state, action_index, reward, next_state):
    ...
    with tf.GradientTape() as tape:
        ...
        loss = keras.losses.mean_squared_error(target_q_values_tensor, q_values) # MSE loss
...
def get_reward(current_state, action):
    ...
    reward +=  (1 / (food_distance + 1e-9)) * 0.01  # Food proximity bonus, added small epsilon to avoid division by zero
    ...
    if action == "up" and current_state[2] == 1 and current_state[1] == 1 or \
       ...
       action == "right" and current_state[2] == 1 and current_state[0] == 1: # Moving straight into danger
        ...
        reward += (min_obstacle_distance_straight / 600) * 0.005 #  Reward proportional to distance from obstacle (scaled down)


    if head.distance(food) < 15:
        reward += 1 + (1 / (food_distance + 1e-9)) * 0.01 # Big positive reward for food + food proximity bonus
